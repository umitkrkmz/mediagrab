...
| `tests/test_docker.py` | `paths.is_docker`, `_ensure_docker_has_a_password` (refuses to start with no password and no `MEDIAGRAB_INITIAL_PASSWORD`, sets one from the env var when missing, leaves a deliberately-disabled password alone), and `lifespan` only running that check inside Docker |
